...
export async function problemRoutes(server: FastifyInstance) {
  ...
  server.post('/', {
    ...
    const { title, description, difficulty, category, estimatedTimeMinutes, testCases } = request.body
  ...
  // Update test case
  server.put('/:id/test-cases/:testCaseId', {
    preHandler: [server.authenticate]
  ...
  // Delete test case
  server.delete('/:id/test-cases/:testCaseId', {
    preHandler: [server.authenticate]
