...
| ID | タスク | 優先度 | 状態 | Issue | 登録 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| T-100 | **Docs**: 初心者向けk8s運用マニュアルの作成 | P2 | ToDo | | - | 未登録 |
| T-101 | 運用マニュアル: `kubectl`の基本コマンドまとめ (Pod一覧、ログ確認、Pod再起動など) | P2 | ToDo | | - | 未登録 |
| T-102 | 運用マニュアル: アプリケーションのデプロイ・更新手順 | P2 | ToDo | | - | 未登録 |
| T-103 | 運用マニュアル: モニタリングダッシュボード(Grafana等)の確認方法 | P3 | ToDo | | - | 未登録 |
| T-104 | 運用マニュアル: 基本的なトラブルシューティング手順 (Podが起動しない、エラーログが出ている等) | P2 | ToDo | | - | 未登録 |
| T-110 | **Docs**: シングルサーバー向けk8sセットアップマニュアルの作成 | P3 | ToDo | | - | 未登録 |
| T-111 | セットアップマニュアル: 軽量k8s(k3s等)のインストールと初期設定手順 | P3 | ToDo | | - | 未登録 |
| T-112 | セットアップマニュアル: Ingress Controllerと永続ストレージの設定手順 | P3 | ToDo | | - | 未登録 |
| T-113 | セットアップマニュアル: 本番用アプリケーションのデプロイ手順 | P3 | ToDo | | - | 未登録 |
